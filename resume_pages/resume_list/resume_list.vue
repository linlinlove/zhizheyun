<template>
	<view class="pages plr-3 box-border mt-1">
		<u-swipe-action v-for="i in list" :key="i.id">
			<u-swipe-action-item :options="options1" @click='clickDel(i.id)'>
				<view class="swipe-action   f-row jc-sb ">
					...
					<view class="_left">
						<view class="one-worp " @click.stop='viewPdf(i.file)'>
							{{i.name}}
						</view>
						<view class="" @click.stop='viewPdf(i.file)'>
							{{i.createtime}}
						</view>
					</view>
		...
		<view class="btn-box" v-if="!isChoice" style="text-align: center;"> 
			<text style="font-size: 30rpx;  color: #c7c6cd;">提示：点击预览，左滑删除</text>
			<!-- font-weight: bold; -->
